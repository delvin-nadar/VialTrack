              ...
              <div className="space-y-3.5">
                ...
                {clientRoutes.length === 0 ? (
                  ...
                  clientRoutes.map((route) => (
                    <div
                      key={route.id}
                      className="bg-white rounded-xl p-4.5 border border-slate-200 shadow-xs space-y-3.5"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="bg-sky-50 text-sky-700 text-xs font-semibold px-2.5 py-0.5 rounded-full border border-sky-200">
                            {route.stops.length} Collection Stops
                          </span>
                          <button
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {route.stops.map((stop, sIdx) => (
                            <div
                              key={stop.id || sIdx}
                              className="bg-slate-50 p-2.5 rounded-lg border border-slate-200 text-xs flex items-start gap-2"
                            >
                              <span className="w-4 h-4 rounded-full bg-sky-700 text-white font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">
                                {sIdx + 1}
                              </span>
                              <div className="min-w-0 flex-1">
                                <div className="font-bold text-slate-900 truncate text-xs">{stop.name}</div>
                                <div className="text-slate-500 text-[11px] truncate">{stop.address}</div>
                                <div className="text-sky-700 text-[10px] mt-0.5">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <span className="text-[10px] text-emerald-800 font-bold uppercase block">
                              Final Destination Diagnostic Lab Handover
                            </span>
                            <span className="font-bold text-slate-900">{route.destinationLab.name}</span>
                          </div>
                        </div>
